...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.show_in_nav_x, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'snatch':
            ...
            break;
        case 'search':
				$uri = $rewrite ? 'tim-kiem' : 'search.php';
            break;
        case 'flow':
