var VTYPEGlobalConfig = {
    method: 0,
    //Chế độ gõ tự động: 0=AUTO, 1=TELEX, 2=VNI, 3=VIQR, 4=VIQR*
    onOff: 1,
    //Tình trạng khởi tạo: 0=Off, 1=On
    ckSpell: 1,
    //Kiểm tra ngôn ngữ: 0=Off, 1=On
    oldAccent: 1,
    //0: Kiểu gõ mới (oa`, oe`, uy`), 1: Kiểu gõ mới (o`a, o`e, u`y)
    useCookie: 1,
    //Lưu cookies: 0=Không lưu, 1=Có lưu
    exclude: ["email"],
    //IDs của trường mà bạn không muốn sử dụng bộ gõ
    showControl: 1,
    //Hiển bị bảng điều khiển: 0=Tắt, 1=Bật. Nếu bạn tắt bảng điều khiển bạn phải tự viết bảng điều khiển riêng cho mình.
    controlCSS: "vtype.css" //Đường dẫn tới tệp vtype.css
};

//Thiết đặt là true với kiểu gõ mà bạn muốn sử dụng khi dùng bộ gõ tự động
var VTYPEAutoConfig = {
    telex: true,
    vni: true,
    viqr: false,
...
function VTYPE() {
    this.radioID = "vtype_auto,vtype_telex,vtype_vni,vtype_viqr,vtype_viqr2,vtype_off,vtype_ckspell,vtype_daucu".split(",");
    this.attached = [];
    this.changed = false;
    ...
    if (VTYPEGlobalConfig.showControl) {
        this.css = document.createElement('link');
        this.css.rel = 'stylesheet';
        this.css.type = 'text/css';
        this.css.href = VTYPEGlobalConfig.controlCSS;
        document.getElementsByTagName('head')[0].appendChild(this.css);
        document.write('<span id="VTYPEControl">');
        document.write('<p class="VTYPEControl"><input id="vtype_auto" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(0);" />Tự động');
        document.write('<input id="vtype_telex" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(1);" />TELEX');
        document.write('<input id="vtype_vni" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(2);" />VNI');
        document.write('<input id="vtype_viqr" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(3);" />VIQR');
        document.write('<input id="vtype_viqr2" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(4);" />VIQR*');
        document.write('<input id="vtype_off" type="radio" name="VTYPEMethod" onclick="VTYPEObj.setMethod(-1);" />Tắt<br />');
        document.write('<a class="VTYPEControl" style="float: right; position: relative; top: 3px;" onclick="document.getElementById(' + "'VTYPEControl').style.display='none';" + '">[Ẩn VTYPE - F12]</a>');
        document.write('<input type="checkbox" id="vtype_ckspell" onclick="VTYPEObj.setSpell(this);" />Chính tả');
        document.write('<input type="checkbox" id="vtype_daucu" onclick="VTYPEObj.setDauCu(this);" />Kiểu cũ</p>');
        document.write('</span>');
    }
    ...
    this.doSetCookie = function () {
        ...
        document.cookie = 'VTYPE_on_off=' + VTYPEGlobalConfig.onOff + ';expires=' + exp;
        document.cookie = 'VTYPE_method=' + VTYPEGlobalConfig.method + ';expires=' + exp;
        document.cookie = 'VTYPE_ckspell=' + VTYPEGlobalConfig.ckSpell + ';expires=' + exp;
        document.cookie = 'VTYPE_daucu=' + VTYPEGlobalConfig.oldAccent + ';expires=' + exp;
    }

    this.doGetCookie = function () {
        var ck = document.cookie,
            res = /VTYPE_method/.test(ck),
            p, i, ckA = ck.split(';');
        if (!res || (ck.indexOf('VTYPE_ckspell') < 0)) {
            this.setCookie();
            return;
        }
        for (i = 0; i < ckA.length; i++) {
            ...
            p[1] = parseInt(p[1]);
            if (p[0] == 'VTYPE_on_off') {
                VTYPEGlobalConfig.onOff = p[1];
            } else if (p[0] == 'VTYPE_method') {
                VTYPEGlobalConfig.method = p[1];
            } else if (p[0] == 'VTYPE_ckspell') {
                if (p[1] == 0) {
                    VTYPEGlobalConfig.ckSpell = 0;
                    this.spellerr = this.nospell;
                } else {
                    VTYPEGlobalConfig.ckSpell = 1;
                    this.spellerr = this.ckspell;
                }
            } else if (p[0] == 'VTYPE_daucu') {
                VTYPEGlobalConfig.oldAccent = parseInt(p[1]);
            }
        }
    }

    if (VTYPEGlobalConfig.useCookie == 1) {
    ...
    this.setMethod = function (m) {
        if (m == -1) {
            VTYPEGlobalConfig.onOff = 0;
            if (this.getEL(this.radioID[5])) {
                this.getEL(this.radioID[5]).checked = true;
            }
        } else {
            VTYPEGlobalConfig.onOff = 1;
            VTYPEGlobalConfig.method = m;
            if (this.getEL(this.radioID[m])) {
                this.getEL(this.radioID[m]).checked = true;
            }
        }
        this.setSpell(VTYPEGlobalConfig.ckSpell);
        this.setDauCu(VTYPEGlobalConfig.oldAccent);
        this.setCookie();
    }

    this.setDauCu = function (box) {
        if (typeof (box) == "number") {
            VTYPEGlobalConfig.oldAccent = box;
            if (this.getEL(this.radioID[7])) {
                this.getEL(this.radioID[7]).checked = box;
            }
        } else {
            VTYPEGlobalConfig.oldAccent = (box.checked) ? 1 : 0;
        }
        this.setCookie();
    }
    ...
    if (this.is_ie || (this.ver >= 1.3) || this.is_opera || this.isKHTML) {
        this.getCookie();
        if (VTYPEGlobalConfig.onOff == 0) this.setMethod(-1);
        else this.setMethod(VTYPEGlobalConfig.method);
        this.setSpell(VTYPEGlobalConfig.ckSpell);
        this.setDauCu(VTYPEGlobalConfig.oldAccent);
    } else {
        this.support = false;
    ...
    this.start = function (obj, key) {
        var w = "",
            method = VTYPEGlobalConfig.method,
            dockspell = VTYPEGlobalConfig.ckSpell,
            uni, uni2 = false,
            uni3 = false,
            uni4 = false;
        this.oc = obj;
        ...
        if (method == 0) {
            var arr = [],
                check = [VTYPEAutoConfig.telex, VTYPEAutoConfig.vni, VTYPEAutoConfig.viqr, VTYPEAutoConfig.viqrStar];
    ...
    this.findC = function (w, k, sf) {
        var method = VTYPEGlobalConfig.method;
        ...
        if ((c == 1) || (uk == this.Z)) {
            ...
            if ((VTYPEGlobalConfig.oldAccent == 0) && ((ttt == "UY") || (ttt == "OA") || (ttt == "OE"))) {
                return vowA[0];
            }
            var c2 = 0,
    ...
    this.ifMoz = function (e) {
        ...
        vtype.ifInit(cwi);
        var node = vtype.range.endContainer,
            newPos;
        vtype.sk = vtype.fcc(code);
        vtype.saveStr = "";
        if (vtype.checkCode(code) || !vtype.range.startOffset || (typeof (node.data) == 'undefined')) return;
        node.sel = false;
        if (node.data) {
            vtype.saveStr = node.data.substr(vtype.range.endOffset);
            if (vtype.range.startOffset != vtype.range.endOffset) {
                node.sel = true;
            }
            node.deleteData(vtype.range.startOffset, node.data.length);
        }
        vtype.range.setEnd(node, vtype.range.endOffset);
        vtype.range.setStart(node, 0);
        if (!node.data) {
            return;
        }
        ...
        node.which = code;
        vtype.start(node, e);
        node.insertData(node.data.length, vtype.saveStr);
        newPos = node.data.length - vtype.saveStr.length + vtype.kl;
        vtype.range.setEnd(node, newPos);
        vtype.range.setStart(node, newPos);
        vtype.kl = 0;
        if (vtype.specialChange) {
            vtype.specialChange = false;
            vtype.changed = false;
    ...
    this.checkCode = function (code) {
        if (((VTYPEGlobalConfig.onOff == 0) || ((code < 45) && (code != 42) && (code != 32) && (code != 39) && (code != 40) && (code != 43)) || (code == 145) || (code == 255))) {
            return true;
        }
    }
    ...
    this.findIgnore = function (el) {
        var va = VTYPEGlobalConfig.exclude,
            i;
    ...
    this.keyDownHandler = function (e) {
        ...
        if (key == 123) {
            document.getElementById('VTYPEControl').style.display = (document.getElementById('VTYPEControl').style.display == 'none') ? 'block' : 'none';
        }
    }
}

function VTYPEInit(VTYPE) {
    var kkk = false;
    if (VTYPE.support && !VTYPE.isKHTML) {
        if (VTYPE.is_opera) {
            if (VTYPE.operaVersion < 9) {
                return;
            }
        }
        for (VTYPE.g = 0; VTYPE.g < VTYPE.fID.length; VTYPE.g++) {
            if (VTYPE.findIgnore(VTYPE.fID[VTYPE.g])) {
                continue;
            }
            if (VTYPE.is_ie) {
                var doc;
                try {
                    VTYPE.frame = VTYPE.fID[VTYPE.g];
                    if (typeof (VTYPE.frame) != "undefined") {
                        if (VTYPE.frame.contentWindow.document) {
                            doc = VTYPE.frame.contentWindow.document;
                        } else if (VTYPE.frame.document) {
                            doc = VTYPE.frame.document;
                        }
                    }
                } catch (e) {}
                if (doc && ((VTYPE.up(doc.designMode) == "ON") || doc.body.contentEditable)) {
                    for (var l = 0; l < VTYPE.attached.length; l++) {
                        if (doc == VTYPE.attached[l]) {
                            kkk = true;
                            break;
                        }
                    }
                    if (!kkk) {
                        VTYPE.attached[VTYPE.attached.length] = doc;
                        VTYPE.attachEvt(doc, "keydown", function () {
                            VTYPE.keyDownHandler("iframe");
                        }, false);
                        VTYPE.attachEvt(doc, "keypress", function () {
                            VTYPE.FKeyPress();
                            if (VTYPE.changed) {
                                VTYPE.changed = false;
                                return false;
                ...
                var iframedit;
                try {
                    VTYPE.wi = VTYPE.fID[VTYPE.g].contentWindow;
                    iframedit = VTYPE.wi.document;
                    iframedit.wi = VTYPE.wi;
                    if (iframedit && (VTYPE.up(iframedit.designMode) == "ON")) {
                        iframedit.VTYPE = VTYPE;
                        VTYPE.attachEvt(iframedit, "keypress", VTYPE.ifMoz, false);
                        VTYPE.attachEvt(iframedit, "keydown", VTYPE.keyDownHandler, false);
                    }
                } catch (e) {}
            }
        }
...
function VTYPEAJAXFix() {
    var a = 50;
    while (a < 5000) {
        setTimeout("VTYPEInit(VTYPEObj)", a);
        a += 50;
    }
}
VTYPEAJAXFix();
VTYPEObj.attachEvt(document, "mousedown", VTYPEAJAXFix, false);
VTYPEObj.attachEvt(document, "keydown", VTYPEObj.keyDownHandler, true);
VTYPEObj.attachEvt(document, "keypress", function (e) {
    var a = VTYPEObj.keyPressHandler(e);
    if (a == false) {
        if (VTYPEObj.is_ie) window.event.returnValue = false;
